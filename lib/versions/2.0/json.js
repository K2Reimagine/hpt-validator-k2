...
import { CHARGE_BILLING_CLASSES, CHARGE_SETTINGS, DRUG_UNITS, } from "./types.js";
...
export async function validateJson(jsonInput, options = {}, mode, outputFilePath, clarifyInfo) {
    ...
    return new Promise(async (resolve) => {
        ...
        parser.onValue = ({ value, key, stack }) => {
            if (typeof key === "string" && stack.length < 2 && isNaN(Number(key))) {
                ...
                if (mode === "FinalizeJson" && outputFilePath) {
                    ...
                    if (typeof value === "object" && !Array.isArray(value) && value) {
                        ...
                        for (let i = 0; i < keys.length; i++) {
                            ...
                            writeStream.write(`${JSON.stringify(nestedKey)}:${JSON.stringify(nestedValue)}`);
                            if (i < keys.length - 1) {
                                writeStream.write(",");
                            }
                            writeStream.write("\n");
            ...
            else if (key === "standard_charge_information" ||
                stack.length > 2) {
                if (mode === "FinalizeJson" && outputFilePath) {
                ...
                hasCharges = true;
                if (mode === "FindIncorrectKeys") {
                    const findIncorrectKeys = (obj, path = "/standard_charge_information") => {
                        Object.entries(obj).forEach(([key, value]) => {
                            const currentPath = `${path}/${key}`;
                            if (!correctJsonKeys.includes(key) &&
                                !extraKeys.some((e) => e.incorrect_attribute_name === key)) {
                                extraKeys.push({
                ...
                if (!validator.validate(STANDARD_CHARGE_SCHEMA, value)) {
                    const validationErrors = validator.errors
                        ...
                        const pathPrefix = stack
                            ...
                            .map((se) => se.key)
                            .join("/");
                        if (error.message.match(/must have required property '(.*?)'/) &&
                            error.message.match(/must have required property '(.*?)'/).length > 0) {
                            const property = error.message.match(/must have required property '(.*?)'/)[1];
                            const path = `${error.path}/${property}`;
                            const transformedPath = transformPath(path);
                            set(value, `${transformedPath}`, null);
                        }
        ...
        parser.onEnd = () => {
            if (!validator.validate(hasCharges ? METADATA_SCHEMA : JSON_SCHEMA, metadata)) {
                const validationErrors = validator.errors.map(enforce2025
                    ? errorObjectToValidationError
                    : errorObjectToValidationErrorWithWarnings);
                const validErrors = validationErrors.filter((err) => err.message.match(/must have required property '(.*?)'/) &&
                    !err.path &&
                    err?.message.match(/must have required property '(.*?)'/)
                        .length > 0);
                validErrors.forEach((error, index) => {
                    const propertyMatch = error.message.match(/must have required property '(.*?)'/);
        ...
        parser.onError = (e) => {
            ...
            resolve({
                ...
                errors: [
                    {
                        path: "",
                        message: `JSON parsing error: ${e.message}.${JSON.stringify(e)}. The validator is unable to review a syntactically invalid JSON file. Please ensure that your file is well-formatted JSON.`,
                    },
                ],
            });
        };
...
function errorObjectToValidationErrorWithWarnings(error, index, errors) {
    ...
    else if (error.schemaPath === "#/definitions/payers_information/allOf/1/if" &&
        index > 0 &&
    ...
    // two validation errors occur for this conditional: one for the "required" keyword, one for the "if" keyword
    else if (error.schemaPath === "#/then/required" &&
        error.params.missingProperty === "drug_information") {
        validationError.warning = true;
    }
    else if (error.schemaPath === "#/if" &&
        index > 0 &&
    ...
    else if (error.instancePath.includes("/drug_information") ||
