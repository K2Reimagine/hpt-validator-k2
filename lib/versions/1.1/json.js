...
import { BILLING_CODE_TYPES, CHARGE_BILLING_CLASSES, CHARGE_SETTINGS, DRUG_UNITS, CONTRACTING_METHODS, } from "./types.js";
...
export async function validateJson(jsonInput, options = {}) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        parser.onEnd = () => {
            ...
            if (!validator.validate(hasCharges ? METADATA_SCHEMA : JSON_SCHEMA, metadata)) {
                valid = false;
                errors.push(...validator.errors.map(errorObjectToValidationError));
            }
            resolve({
                valid,
                errors: options.maxErrors && options.maxErrors > 0
